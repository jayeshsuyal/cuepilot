import { ClientError, receiptMessage } from "../lib/model";
import type {
  CuePilotClient,
  CueReceipt,
  Run,
...
const KEY = "cuepilot.explicit-fixture.contract-v1";
...
function initial(): Store {
  return {
    ...
    show: {
      id: "fixture-demo",
      revision: 1,
      title: "CuePilot — From rehearsal to recall",
      speakers: [
        {
          id: "maya",
          name: "Maya Chen",
...
function load(): Store {
  ...
  try {
    ...
    throw new ClientError(
      "Saved fixture data is unreadable. Clear the CuePilot fixture entry in browser storage to start again.",
      "FIXTURE_STORAGE",
    );
  }
}
...
export const fixtureClient: CuePilotClient = {
  ...
  createRun(speakerId, executionMode, notes) {
    return mutate(async (s) => {
      ...
      const digest = await crypto.subtle.digest(
        "SHA-256",
        new TextEncoder().encode(JSON.stringify({ plan, notes, id })),
      );
  ...
  advance(id, requestId) {
    return mutate((s) => {
      const key = `${id}:${requestId}`;
      if (s.requests[key]) return receiptMessage(s.requests[key]);
      const run = requireRun(s, id);
      ...
      const cue = run.plan.cues.find((c) => c.index === run.nextStep);
      if (!cue)
        throw new ClientError(
